...
beam = hps.Beam(mass=939.294, charge=-1.0, current=0.015, num=1024*1) #H- beam
    ...
    # simulate here
        ...
        if len(mask) > 0:
            try:
                ...
                plot.profile(  'x', beam, mask, 7, 'r-')
                plot.profile(  'y', beam, mask, 8, 'r-')
                plot.profile('phi', beam, mask, 9, 'r-')
                plot.profile( 'xp', beam, mask, 10, 'r-')
                plot.profile( 'yp', beam, mask, 11, 'r-')
                plot.profile(  'w', beam, mask, 12, 'r-')
